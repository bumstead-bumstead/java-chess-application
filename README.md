...
- [x] 예외 정의
  ...
  4. 해당 기물이 움직일 수 없는 곳으로 이동 시도
   ...
   - [x] 예외 더 세분화하기
   - [x] Piece 팩토리 클래스 만들기
   - [x] Board.verifyBlockedByPiece() 리팩토링...
   - [x] verifyTurn이 빈 객체 움직이는 경우도 잡아버리는 문제 fix
   - [ ] Piece queen, Rook, Bishop / others 인터페이스 분리하기?
   - [ ] 테스트코드 인터페이스 뽑아내기 시도해보기 그리고 공유하기
   - [ ] Piece들 이동 verify 로직에 스트림 쓰기
   - [ ] 테스트코드 리팩토링 : parameterized test 적용하고, 테스트 이름 붙이기
   - [ ] king 잡으면 게임 종료 기능
   - [ ] processRun() -> 재귀로 짜면 위험하다.
   - [ ] 예외 나누는 게 의미가 없는 것 같다.
   - [ ] turn -> chessGame에서 유지하기.. 훨씬 편해진다
...
- turn 변수는 game에서 유지하는게 맞지 않나?? 아닌 것 같기도
